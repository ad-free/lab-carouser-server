...
from apps.apis.serializers.api_friend_anonymous import AnonymousFriendSerializer
...
class AnonymousFriend(APIView):
	""" Update profile """
	
	authentication_classes = [TokenAuthentication]
	permission_classes = [IsAuthenticated & partial(APIAccessPermission, API().get_api_name('friend', 'anonymous'))]
	...
	def post(self, request):
		serializer = AnonymousFriendSerializer(data=self.request.data)
		
		if serializer.is_valid():
			page = serializer.data['page']
			obj_friends = Friend.objects\
				.distinct()\
				.filter(is_accept=False)\
				.values('id', 'first_name', 'last_name', 'sex', 'email', 'is_accept')\
