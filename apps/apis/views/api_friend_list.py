...
from apps.apis.serializers.api_friend_list import ListFriendSerializer
...
class ListFriend(APIView):
	...
	def post(self, request):
		serializer = ListFriendSerializer(data=self.request.data)
		...
		if serializer.is_valid():
			...
			obj_friends = self.request.user.relationship_with\
				.distinct()\
				.all()\
				.values('id', 'first_name', 'last_name', 'sex', 'email')\
